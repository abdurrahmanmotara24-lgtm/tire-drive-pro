...
function Index() {
  ...
  const heroBg = hero.background_image || tireStack;
  const callHref = `tel:${contact.phone.replace(/[^+\d]/g, "") || "10000000000"}`;
        ...
        <div className="container-tny grid gap-10 py-16 lg:grid-cols-12 lg:items-center lg:py-24">
          <div className="lg:col-span-7 text-primary-foreground">
            <span className="inline-flex items-center gap-2 rounded-full bg-brand-red/95 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-brand-red-foreground shadow-red">
              <BadgeCheck className="h-3.5 w-3.5" /> {hero.badge}
            </span>
            <h1 className="mt-5 text-balance text-4xl font-bold leading-[1.05] tracking-tight text-primary-foreground sm:text-5xl lg:text-6xl">
              {hero.title_line1}
              <br />
              <span className="text-brand-red">{hero.title_line2}</span>
            </h1>
            ...
            <div className="mt-6 flex flex-wrap gap-2.5">
              ...
              >
                {hero.cta_primary_text} <ArrowRight className="h-4 w-4" />
              </a>
              <a
              ...
              >
                <Phone className="h-4 w-4" /> {hero.cta_secondary_text}
              </a>
            </div>
            <div className="mt-8 grid max-w-md grid-cols-3 gap-4">
              {hero.stats.map((s) => (
                <div key={s.label}>
                  <div className="text-xl font-bold text-brand-red sm:text-2xl">{s.value}</div>
                  <div className="text-[11px] uppercase tracking-wider text-primary-foreground/75">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-accent-gradient py-10 text-brand-red-foreground">
          <div className="container-tny flex flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left">
            ...
            <div className="flex flex-wrap justify-center gap-2.5">
              <a href={callHref} className="inline-flex items-center gap-2 rounded-full bg-background px-5 py-2.5 text-sm font-semibold text-brand-red">
